...
`internal/gen` is not checked in and nothing in go-boot writes it, so **this
service does not compile until `buf generate` has run once.** gRPC shares the
HTTP listener, so there is no second port to open.
...
| `proto/` | the sample `.proto`. Rename it or delete it — go-boot never reads it |
| `buf.yaml`, `buf.gen.yaml` | what `buf generate` reads. There is no Makefile: go-boot does not own your build tool |

The feature also has an `rpc/` package, the gRPC adapter and its own `Routes`
mount, a sibling of `rest/` — both are shells over the same Service Layer. It
is called `rpc` and not `grpc` so that the go-boot `grpc` package it needs does
not have to be imported under an alias. It is a separate type from
the Service Layer because connect-go's generated interface wants
`Greet(ctx, *connect.Request[...])` and `Service` already owns the name `Greet`.
