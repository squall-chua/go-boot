...
//
// go-boot has no such type and never will. Its Transport takes an
// http.Handler and nothing else (ADR 0004), because a handler signature in the
// library would need an adapter for every piece of net/http middleware ever
// written. This adapter is YOURS — it lives in your module, you can change it,
// and go-boot only ever sees the plain http.Handler that Handle returns.
package transport
...
// StatusError is how a handler asks for a status other than 500. Anything
// else it returns is logged and answered with a bare 500, so an error from
// deep below — a driver naming the host and user a password failed for —
// cannot reach the caller by accident.
...
// Handle joins the two halves and returns the http.Handler that routes.go
// mounts. Success is 200. A route that must answer 201 or 204 mounts an
// ordinary http.HandlerFunc instead — go-boot takes any http.Handler, so this
// adapter is a convenience, never a wall.
...
// NoBody is the Bind for a route that reads nothing from the request.
func NoBody(*http.Request) (struct{}, error) { return struct{}{}, nil }
