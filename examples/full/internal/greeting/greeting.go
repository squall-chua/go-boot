...
// and the behaviour built on it. It imports no database driver, no HTTP package
// and no connect package, and nothing here knows a request exists.
//
...
//	greeting/rpc/      gRPC: the connect adapter and its mount
//
// Both Transports call the same Service, and BOTH FORMS OF MAIN wire the same
// one. That is the point of the split: the Preset form and the explicit form
// differ in wiring, never in behaviour.
package greeting
...
// domain says what it wants, and an adapter supplies it. greeting_test.go swaps
// in a fake in four lines, with no PostgreSQL anywhere.
